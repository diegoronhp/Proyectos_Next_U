<?php
  ...
  class ConectorBD
  {
    ...
    function getEventsUser($user_id){
            $sql = "SELECT ev.tituloEvento AS Titulo, ev.fechaInicio AS FechaInicial, ev.horaInicio AS HoraInicial,
                    ev.fechaFin AS FechaFinal, ev.horaFin AS HoraFinal, ev.diaCompleto AS DiaCompleto, ev.idEvento AS Id
                    FROM evento AS ev
                    JOIN usuario AS us
                    ON us.idUsuario = ev.fkUsuario
                    WHERE us.idUsuario = ".$user_id.";";
       return $this->ejecutarQuery($sql);
    }
  }
 ?>
